...
export default function TercerTiempoPOS() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Build inventory lookup: "productId|cutId|size|color" -> stock
      const invMap = {};
      iRes.data.forEach(row => {
        invMap[`${row.product_id}|${row.cut_id}|${row.size}|${row.color}`] = row.stock;
      });
      setInventory(invMap);
      ...
      const salesMapped = sRes.data.map(s => ({
        id: s.id,
        date: s.created_at,
        customer: s.customer_name || "Sin nombre",
        ...
        items: (s.sale_items || []).map(si => ({
          ...
          name: pRes.data.find(p => p.id === si.product_id)?.name || "?",
  ...
  const refreshInventory = useCallback(async () => {
    ...
    if (!error && data) {
      const invMap = {};
      data.forEach(row => { invMap[`${row.product_id}|${row.cut_id}|${row.size}|${row.color}`] = row.stock; });
      setInventory(invMap);
    }
  }, []);
  ...
  const applyCoupon = useCallback(() => {
    ...
    if (found) {
      setAppliedCoupon({ type: found.type, value: parseFloat(found.value), code: found.code, label: found.label });
      setCouponError(""); setManualDiscount("");
    } else {
      setCouponError("Cupón no válido"); setAppliedCoupon(null);
    }
  }, [couponCode, couponsDB]);

  // ─── COMPLETE SALE → WRITE TO SUPABASE ───────────────────
  ...
  const completeSale = useCallback(async () => {
    ...
    try {
      ...
      if (email) {
        ...
        // No email but has name — create without unique constraint
        const { data: newCust, error: custErr } = await supabase.from("customers").insert({
      ...
      // 3. Insert sale items (trigger deducts inventory automatically)
      ...
      if (appliedCoupon?.code) {
        await supabase.rpc("increment_coupon_uses", undefined).catch(() => {
          // If RPC doesn't exist, just update directly
          supabase.from("coupons").update({ uses: couponsDB.find(c => c.code === appliedCoupon.code)?.uses + 1 || 1 }).eq("code", appliedCoupon.code);
        });
      }
  ...
  }, [canCompleteSale, cart, cartSubtotal, discountAmount, cartTotal, appliedCoupon, paymentMethod, effectiveStatus, customerName, customerEmail, customerPhone, customerAddress, saleNotes, couponsDB, loadData]);

  // ─── UPDATE SALE STATUS → SUPABASE ───────────────────────
  ...
  const adjustInventory = useCallback(async (key, delta) => {
    ...
    // Optimistic update
    setInventory(prev => ({ ...prev, [key]: newStock }));
    ...
    if (error) {
      // Revert on error
      setInventory(prev => ({ ...prev, [key]: currentStock }));
      console.error("Inventory update error:", error);
    }
  }, [inventory]);

  const filteredProducts = useMemo(() => { if (!searchTerm) return products; const t = searchTerm.toLowerCase(); return products.filter(p => p.name.toLowerCase().includes(t) || p.tag.toLowerCase().includes(t)); }, [searchTerm, products]);
  ...
  if (!dbReady && !dbError) {
    return (
      <div style={{ fontFamily: "'Rajdhani', sans-serif", background: "#0D1B0F", color: "#F5F0E1", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
        ...
        <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 42, letterSpacing: 4, color: "#C9A227" }}>TERCER TIEMPO</div>
        <div style={{ fontSize: 14, opacity: 0.5, letterSpacing: 2 }}>CONECTANDO CON BASE DE DATOS...</div>
  ...
  if (dbError) {
    return (
      <div style={{ fontFamily: "'Rajdhani', sans-serif", background: "#0D1B0F", color: "#F5F0E1", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, padding: 40 }}>
        ...
        <button onClick={loadData} style={{ background: "#C9A227", color: "#0D1B0F", border: "none", borderRadius: 10, padding: "10px 24px", fontFamily: "'Bebas Neue', sans-serif", fontSize: 18, letterSpacing: 2, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
    ...
    <div style={{ fontFamily: "'Rajdhani', 'Segoe UI', sans-serif", background: "#0D1B0F", color: "#F5F0E1", minHeight: "100vh", display: "flex", flexDirection: "column", overflow: "hidden", fontSize: 15 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #1A472A 0%, #0D1B0F 100%)", borderBottom: "2px solid #C9A227", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexShrink: 0 }}>
        <div>
          ...
          <div style={{ fontSize: 10, letterSpacing: 5, color: "#F5F0E199", fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
            SISTEMA DE VENTAS
            {syncing && <span style={{ color: "#C9A227", display: "flex", alignItems: "center", gap: 3 }}><Icon name="refresh" size={10} /> SYNC</span>}
            {!syncing && <span style={{ color: "#4a8c5c", display: "flex", alignItems: "center", gap: 3 }}><Icon name="wifi" size={10} /></span>}
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {[
            ...
            <button key={tab.id} onClick={() => { setView(tab.id); if (tab.id === "inv" || tab.id === "dash") loadData(); }} style={{
      ...
      {saleComplete && (
        <div style={{ position: "fixed", top: 80, left: "50%", transform: "translateX(-50%)", background: "#1A472A", border: "2px solid #C9A227", borderRadius: 16, padding: "16px 32px", zIndex: 999, display: "flex", alignItems: "center", gap: 12, boxShadow: "0 20px 60px rgba(0,0,0,0.6)", animation: "slideDown 0.3s ease" }}>
          ...
          <div><div style={{ fontWeight: 700, fontSize: 18, fontFamily: "'Bebas Neue', sans-serif", letterSpacing: 2 }}>VENTA REGISTRADA</div><div style={{ fontSize: 12, opacity: 0.7 }}>Guardada en base de datos</div></div>
        </div>
      )}

      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        {view === "pos" && <POSView {...{ products: filteredProducts, cuts, searchTerm, setSearchTerm, selectedProduct, setSelectedProduct, cart, addToCart, addApartadaToCart, updateCartQty, removeFromCart, getStock, cartSubtotal, cartTotal, discountAmount, couponCode, setCouponCode, applyCoupon, appliedCoupon, setAppliedCoupon, couponError, setCouponError, manualDiscount, setManualDiscount, paymentMethod, setPaymentMethod, orderStatus, setOrderStatus, effectiveStatus, availableStatuses, hasApartadaItems, customerName, setCustomerName, customerEmail, setCustomerEmail, customerPhone, setCustomerPhone, customerAddress, setCustomerAddress, saleNotes, setSaleNotes, completeSale, canCompleteSale, fmt, posGender, setPosGender, syncing }} />}
        {view === "orders" && <OrdersView {...{ sales, updateSaleStatus, salesFilter, setSalesFilter, fmt, fmtTime }} />}
        {view === "inv" && <InventoryView {...{ inventory, adjustInventory, invFilter, setInvFilter, invGender, setInvGender, fmt, cuts, products }} />}
...
// ─── POS VIEW ──────────────────────────────────────────────────────────
function POSView({ products, cuts, searchTerm, setSearchTerm, selectedProduct, setSelectedProduct, cart, addToCart, addApartadaToCart, updateCartQty, removeFromCart, getStock, cartSubtotal, cartTotal, discountAmount, couponCode, setCouponCode, applyCoupon, appliedCoupon, setAppliedCoupon, couponError, setCouponError, manualDiscount, setManualDiscount, paymentMethod, setPaymentMethod, orderStatus, setOrderStatus, effectiveStatus, availableStatuses, hasApartadaItems, customerName, setCustomerName, customerEmail, setCustomerEmail, customerPhone, setCustomerPhone, customerAddress, setCustomerAddress, saleNotes, setSaleNotes, completeSale, canCompleteSale, fmt, posGender, setPosGender, syncing }) {
                  ...
                  {SIZES.map(size => COLORS.map(color => {
                    ...
                    return (
                      <button key={`${size}-${color}`} onClick={() => isOut ? addApartadaToCart(selectedProduct, cut, size, color) : addToCart(selectedProduct, cut, size, color)}
                        style={{ background: isOut ? "#8B691420" : color === "Negro" ? "#18181b" : "#F5F0E112", border: isOut ? "2px dashed #8B691466" : isLow ? "2px solid #C9A22788" : "1px solid #F5F0E125", borderRadius: 10, padding: "6px 4px", cursor: "pointer", color: "#F5F0E1", opacity: isOut ? 0.7 : 1, textAlign: "center", fontFamily: "'Rajdhani', sans-serif", position: "relative", transition: "all 0.15s" }}>
                        ...
                        {isOut ? <div style={{ fontSize: 8, fontWeight: 800, color: "#8B6914", display: "flex", alignItems: "center", justifyContent: "center", gap: 2 }}><Icon name="lock" size={8} /> APARTAR</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            ...
            {hasApartadaItems && (
              <div style={{ animation: "fadeIn 0.2s ease", display: "flex", flexDirection: "column", gap: 5 }}>
                ...
                <div style={{ display: "flex", alignItems: "start", gap: 5 }}><span style={{ color: "#8B6914", flexShrink: 0, marginTop: 8 }}><Icon name="mappin" size={14} /></span><textarea value={customerAddress} onChange={e => setCustomerAddress(e.target.value)} placeholder="Dirección de envío *" rows={2} style={{ ...inputStyle, resize: "none", lineHeight: 1.4 }} /></div>
              </div>
            )}
            <div style={{ display: "flex", alignItems: "start", gap: 5 }}><span style={{ color: "#F5F0E144", flexShrink: 0, marginTop: 8 }}><Icon name="note" size={14} /></span><textarea value={saleNotes} onChange={e => setSaleNotes(e.target.value)} placeholder="Notas (opcional)" rows={1} style={{ ...inputStyle, resize: "none", lineHeight: 1.4, fontSize: 13 }} /></div>
          </div>
        </div>
        <div style={{ flex: 1, overflow: "auto", padding: "8px 12px" }}>
          {cart.length === 0 ? (
            <div style={{ textAlign: "center", padding: 40, opacity: 0.3 }}><Icon name="rugby" size={40} /><div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 18, letterSpacing: 2, marginTop: 12 }}>SIN PRODUCTOS</div></div>
          ) : cart.map(item => (
            <div key={item.key} style={{ background: item.isApartada ? "#8B691415" : "#F5F0E108", borderRadius: 12, padding: 12, marginBottom: 8, border: item.isApartada ? "1px solid #8B691433" : "1px solid #F5F0E110", animation: "fadeIn 0.15s ease" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, lineHeight: 1.2, display: "flex", alignItems: "center", gap: 6 }}>{item.name}{item.isApartada && <span style={{ background: "#8B691440", borderRadius: 4, padding: "1px 6px", fontSize: 9, fontWeight: 800, color: "#8B6914" }}>APARTADO</span>}</div>
                  <div style={{ fontSize: 11, opacity: 0.5, marginTop: 2, display: "flex", alignItems: "center", gap: 4 }}>{item.cut} · {item.size} · <span style={{ width: 8, height: 8, borderRadius: 99, background: item.color === "Negro" ? "#333" : "#F5F0E1", border: "1px solid #F5F0E133", display: "inline-block" }} /> {item.color}</div>
                </div>
        ...
        {cart.length > 0 && (
          <div style={{ borderTop: "2px solid #C9A227", padding: 14, background: "#1A472A15", overflow: "auto", maxHeight: "45vh" }}>
            <div style={{ marginBottom: 8 }}>
              ...
              {hasApartadaItems && <div style={{ fontSize: 10, color: "#8B6914", marginTop: 4, fontWeight: 600 }}>Productos agotados — se registra como apartado</div>}
            </div>
            <div style={{ marginBottom: 8 }}>
              {!appliedCoupon ? (
                <div style={{ display: "flex", gap: 6 }}>
              ...
              {!appliedCoupon && <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}><span style={{ fontSize: 11, opacity: 0.5 }}>Desc. manual $</span><input value={manualDiscount} onChange={e => setManualDiscount(e.target.value.replace(/\D/g, ""))} placeholder="0" style={{ width: 65, padding: "3px 8px", background: "#F5F0E10A", border: "1px solid #F5F0E120", borderRadius: 6, color: "#F5F0E1", fontFamily: "'Rajdhani', sans-serif", fontSize: 14, fontWeight: 700, textAlign: "right" }} /></div>}
            </div>
            ...
            <button onClick={completeSale} disabled={!canCompleteSale || syncing} style={{
              ...
              <Icon name={syncing ? "refresh" : ORDER_STATUSES[effectiveStatus].icon} size={22} />
              {syncing ? "GUARDANDO..." : effectiveStatus === "pagada" ? "COBRAR" : effectiveStatus === "porEntregar" ? "REGISTRAR" : "APARTAR"} {fmt(cartTotal)}
            </button>
            {!canCompleteSale && hasApartadaItems && <div style={{ fontSize: 10, color: "#8B6914", marginTop: 6, textAlign: "center", fontWeight: 600 }}>Completa nombre, email, teléfono y dirección para apartar</div>}
          </div>
        )}
      </div>
    </div>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 16 }}>
        {filtered.length === 0 ? (
          ...
          return (
            <div key={sale.id} style={{ background: "#F5F0E108", border: `1px solid ${st.color}33`, borderRadius: 14, padding: 16, marginBottom: 10 }}>
              ...
              {sale.notes && (
                <div style={{ background: "#F5F0E108", border: "1px solid #F5F0E115", borderRadius: 8, padding: "6px 12px", marginBottom: 10, fontSize: 12, display: "flex", alignItems: "start", gap: 6, opacity: 0.7 }}>
                  <span style={{ flexShrink: 0, marginTop: 1 }}><Icon name="note" size={12} /></span><span>{sale.notes}</span>
                </div>
              )}
...
// ─── INVENTORY VIEW ────────────────────────────────────────────────────
function InventoryView({ inventory, adjustInventory, invFilter, setInvFilter, invGender, setInvGender, fmt, cuts, products }) {
  ...
  const getProductValue = (pid) => Object.entries(inventory).filter(([k]) => k.startsWith(`${pid}|`) && (invGender === "all" || genderCuts.some(c => k.includes(c.id)))).reduce((s, [k, q]) => { const cut = cuts.find(c => k.includes(c.id)); return s + (cut ? parseFloat(cut.price) * q : 0); }, 0);
      ...
      <div style={{ display: "flex", gap: 6, padding: "10px 16px", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", background: "#F5F0E10A", borderRadius: 8, overflow: "hidden", border: "1px solid #F5F0E120" }}>
          {[["all", "Todos"], ["hombre", "Hombre"], ["mujer", "Mujer"]].map(([g, label]) => (
            <button key={g} onClick={() => setInvGender(g)} style={{ padding: "6px 14px", border: "none", cursor: "pointer", fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, fontSize: 12, background: invGender === g ? (g === "hombre" ? "#3a6b8c" : g === "mujer" ? "#8B4566" : "#C9A227") : "transparent", color: invGender === g ? "#fff" : "#F5F0E166" }}>{label}</button>
          ))}
        </div>
      </div>
          ...
          return (
            <div key={product.id} style={{ marginBottom: 8 }}>
              <button onClick={() => setExpandedProduct(expandedProduct === product.id ? null : product.id)}
                ...
                <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 15 }}>{product.name}</div><div style={{ fontSize: 11, opacity: 0.4 }}>{genderCuts.map(c => c.short_label).join(" · ")}</div></div>
            ...
            <button key={sale.id} onClick={() => setShowSaleDetail(showSaleDetail === sale.id ? null : sale.id)} style={{ width: "100%", background: showSaleDetail === sale.id ? "#F5F0E10F" : "transparent", border: "none", borderBottom: "1px solid #F5F0E110", padding: "10px 0", cursor: "pointer", color: "#F5F0E1", textAlign: "left" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, opacity: 0.4 }}>{sale.email || "Sin email"}</div>
                </div>
                ...
                <div style={{ marginTop: 8, padding: "8px 12px", background: "#F5F0E108", borderRadius: 8, marginLeft: 40 }}>
                  {sale.items.map((item, idx) => (
                    <div key={idx} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "2px 0" }}>
                      <span>{item.name} · {item.cut} · {item.size} · {item.color} x{item.qty}</span>
                      <span>{fmt(item.price * item.qty)}</span>
                    </div>
                  ))}
                  {sale.notes && <div style={{ fontSize: 11, opacity: 0.5, borderTop: "1px solid #F5F0E110", marginTop: 4, paddingTop: 4 }}><Icon name="note" size={10} /> {sale.notes}</div>}
                </div>
              )}
            </button>
        ); })}
